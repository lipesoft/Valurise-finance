...
export const legalDocuments: Record<"privacy" | "terms" | "cookies", LegalDocument> = {
  privacy: {
    ...
    sections: [
      ...
      { heading: "Armazenamento e prestadores", paragraphs: ["A aplicação é hospedada na Vercel e os serviços de autenticação e banco de dados usam Supabase. Esses prestadores processam dados para fornecer infraestrutura, segurança e disponibilidade, conforme seus contratos e políticas.", "Se você conectar OpenAI, Gemini ou DeepSeek e habilitar o uso de contexto financeiro, a Valurise envia ao provedor escolhido mensagens e um resumo limitado dos dados necessários à resposta. O uso fica sujeito também às condições e políticas do provedor. Você pode desligar os insights ou remover a conexão nas Configurações."] },
  ...
  terms: {
    ...
    sections: [
      ...
      { heading: "Recursos de IA", paragraphs: ["A IA é opcional e só deve receber contexto financeiro quando você conectar um provedor e habilitar essa opção. Respostas podem estar incompletas ou incorretas: confirme valores e não use a IA como única base para decisões financeiras. A Valurise não realiza movimentações financeiras em seu nome."] },
