...
function BusinessWorkspaceWelcome({ displayName, openAccounts, openFinancialProfile, continueToDashboard }: { displayName: string; openAccounts: () => void; openFinancialProfile: () => void; continueToDashboard: () => void }) {
...
function App({ user, workspace, workspaces, onSwitchWorkspace, onRegisterBeforeWorkspaceSwitch, onCreateWorkspace, logout, onLoadingStatusChange }: { user: User; workspace: WorkspaceSummary; workspaces: WorkspaceSummary[]; onSwitchWorkspace: (workspaceId: string) => void; onRegisterBeforeWorkspaceSwitch: (flush: () => Promise<void>) => void; onCreateWorkspace: () => void; logout: () => void; onLoadingStatusChange: (status: ValuriseSplashStatus) => void }) {
  ...
  if (!data.onboarded)
    return workspace.type === "personal" ? (
      ...
      />
    ) : <BusinessWorkspaceWelcome displayName={workspace.displayName} openAccounts={() => { saveData({ ...data, onboarded: true }); setView("accounts"); }} openFinancialProfile={() => { saveData({ ...data, onboarded: true }); setView("settings"); }} continueToDashboard={() => saveData({ ...data, onboarded: true })} />;
