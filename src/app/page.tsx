...
function MasterUsers({ toast }: { toast: (text: string) => void }) {
  ...
  const pendingCount = users.filter((item) => item.profile?.account_status === "pending").length;
  return <section><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><b className="text-lg">Usuários e convites</b>{pendingCount > 0 && <span className="rounded-full bg-[var(--accent)] px-2 py-0.5 text-[10px] font-bold text-[var(--accentfg)]">{pendingCount} pendente{pendingCount > 1 ? "s" : ""}</span>}</div><p className="muted mt-1 text-xs">Aprova acessos e administra contas sem ler dados financeiros.</p></div><button onClick={() => void load()} className="shrink-0 text-xs text-[var(--accent)]">Atualizar</button></div><div className="mt-5 rounded-2xl bg-[var(--panel2)] p-4"><div className="flex items-center justify-between gap-3"><span><b className="block text-sm">Convidar novo usuário</b><small className="muted block pt-1">O cadastro pelo link continua sujeito à sua aprovação.</small></span><button onClick={() => void createInvite()} className="shrink-0 rounded-xl border border-[var(--accent)] px-3 py-2 text-xs font-semibold text-[var(--accent)]">Gerar link</button></div>{inviteLink && <div className="mt-4 flex gap-2"><input aria-label="Link de convite" readOnly value={inviteLink} className="field min-w-0 flex-1 text-xs"/><button onClick={async () => { await navigator.clipboard?.writeText(inviteLink); toast("Link copiado."); }} className="rounded-xl bg-[var(--panel)] px-3 text-xs">Copiar</button></div>}</div>{loading ? <p className="muted mt-5 text-sm">Carregando usuários…</p> : <div className="mt-4 max-h-[28rem] space-y-2 overflow-y-auto pr-1">{users.map((item) => { const status = item.profile?.account_status || "pending"; const label = item.profile?.full_name || item.email || "Usuário"; const working = busy?.startsWith(item.id); const confirmingDelete = deleteCandidate === item.id; const isMasterAccount = item.profile?.account_role === "master"; return <article key={item.id} className="rounded-2xl border border-[var(--border)] bg-[var(--panel2)]/65 p-4"><div className="flex items-start justify-between gap-3"><span className="min-w-0"><b className="block truncate text-sm">{label}</b><small className="muted mt-1 block truncate">{item.email}</small><small className="mt-1 block text-[11px] capitalize text-[var(--accent)]">{isMasterAccount ? "Conta Master" : status}</small></span>{status === "pending" && !isMasterAccount && <button disabled={working} onClick={() => void act(item.id, "approve")} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-xs font-semibold text-[var(--accentfg)]">Aprovar acesso</button>}</div>{!isMasterAccount && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs"><button disabled={working} onClick={() => void act(item.id, status === "trashed" ? "restore" : "trash")} className="muted hover:text-[var(--fg)]">{status === "trashed" ? "Restaurar da lixeira" : "Mover para lixeira"}</button>{status !== "trashed" && <button disabled={working} onClick={() => void act(item.id, status === "disabled" ? "restore" : "disable")} className="muted hover:text-[var(--fg)]">{status === "disabled" ? "Reativar" : "Desativar"}</button>}{status === "trashed" && (confirmingDelete ? <><button disabled={working} onClick={() => { setDeleteCandidate(null); void act(item.id, "delete_permanently"); }} className="font-medium text-[var(--danger)]">Confirmar exclusão definitiva</button><button disabled={working} onClick={() => setDeleteCandidate(null)} className="muted">Cancelar</button></> : <button disabled={working} onClick={() => setDeleteCandidate(item.id)} className="text-[var(--danger)]">Excluir definitivo</button>)}</div>}{confirmingDelete && <p className="mt-3 text-xs text-[var(--danger)]">Esta ação apaga a conta e todos os dados financeiros dela.</p>}</article>; })}</div>}</section>;
}
function Dashboard({
  user,
  sum,
